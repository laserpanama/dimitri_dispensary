...
## 2025-05-23 - [Broken Access Control in Chat Router]
**Vulnerability:** The `updateAgentStatus` procedure was incorrectly using `protectedProcedure`, allowing any authenticated user to update their status as a chat agent, regardless of their role.
**Learning:** Procedures that handle role-specific logic (like "agents") must be explicitly protected by role-based middleware. Relying on generic authentication checks for administrative actions leads to privilege escalation.
**Prevention:** Default to `adminProcedure` for all endpoints that modify system-wide state or agent-specific metadata. Always include negative test cases for unauthorized roles in security test suites.
