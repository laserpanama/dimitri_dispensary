...
## 2025-05-22 - [Redundant State Syncing and Recalculation in Cart]
**Learning:** Derived data (like lookup maps from fetched arrays) and expensive calculations (like order totals) should use `useMemo` instead of the `useState` + `useEffect` pattern. This avoids an extra render cycle for synchronization and prevents redundant O(N) operations when unrelated component state (e.g., fulfillment type) changes.
**Action:** Use `useMemo` for any data that can be derived directly from existing props or state.
