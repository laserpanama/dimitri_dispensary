...
## 2025-05-20 - [Optimizing Derived State in React Components]
**Learning:** Using `useState` and `useEffect` to sync derived state (like a product lookup from a list) causes an unnecessary extra render cycle. Deriving the lookup object synchronously using `useMemo` during the render phase is more efficient and ensures data consistency within a single render.
**Action:** Always prefer `useMemo` for deriving state from props or other state variables over the `useState` + `useEffect` pattern.
