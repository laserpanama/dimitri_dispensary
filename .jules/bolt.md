...
## 2025-05-18 - [Derived Data Optimization in Cart Component]
**Learning:** Using `useEffect` + `useState` to synchronize derived data (like creating a lookup object from an array) is a performance anti-pattern that triggers an unnecessary second render cycle. In the Cart component, this caused redundant re-renders whenever products were fetched. Additionally, calculating the total price on every render caused overhead during text input in the delivery address field.
**Action:** Always prefer `useMemo` for derived data and lookup objects. Use the project's `usePersistFn` hook for stable event handler references to prevent child component re-renders without the complexity of `useCallback` dependency arrays.
