...
export default function Cart() {
  ...
  // Optimization: Stabilize the dependency key by using a sorted join string.
  // This ensures we only re-fetch if the SET of product IDs changes, not just quantities.
  const cartProductIdsKey = useMemo(
    () => cartItems.map((item) => item.productId).sort().join(","),
    [cartItems]
  );
  const cartProductIds = useMemo(
    () => (cartProductIdsKey ? cartProductIdsKey.split(",").map(Number) : []),
    [cartProductIdsKey]
  );
  ...
  // Optimization: Derive product lookup directly from query data to eliminate extra render cycles.
  ...
  // Optimization: Memoize total price calculation to avoid re-computing on every render.
  const total = useMemo(() => {
