...
describe("Chat Validation & RBAC", () => {
  ...
  it("should reject subjects longer than 255 characters", async () => {
    const { ctx } = createAuthContext(1);
    const caller = appRouter.createCaller(ctx);
  ...
  it("should prevent non-admin from accessing active conversations via adminProcedure", async () => {
  ...
  it("should allow admin to access active conversations", async () => {
    const { ctx } = createAuthContext(1, "admin");
    const caller = appRouter.createCaller(ctx);

    vi.spyOn(chatDb, 'getActiveConversations').mockResolvedValue([]);

    const result = await caller.chat.getActiveConversations();
    expect(result).toEqual([]);
  });

  it("should reject negative or zero conversationId", async () => {
    const { ctx } = createAuthContext(1);
    const caller = appRouter.createCaller(ctx);

    try {
      await caller.chat.getMessages({ conversationId: 0 });
      expect.fail("Should have thrown validation error");
    } catch (error: any) {
       // Zod error for positive()
